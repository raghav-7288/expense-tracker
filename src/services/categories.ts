...
// ============================================
// These maintain the same API signature for existing consumers
// (Transaction forms, filters, etc.)

/** @deprecated Use getMergedCategories instead */
export async function getCategories(userId: string, type?: TransactionType) {
  return getMergedCategories(userId, type);
}

/** @deprecated Use createUserCategory instead */
export async function createCategory(input: CreateCategoryInput) {
  return createUserCategory(input);
}

/** @deprecated Use updateUserCategory instead */
export async function updateCategory(id: string, input: UpdateCategoryInput) {
  // We can't verify ownership here without userId, 
  // but RLS will enforce it server-side
  const { data, error } = await supabase
    .from('user_categories')
    .update({
      ...(input.name !== undefined && { name: input.name.trim() }),
